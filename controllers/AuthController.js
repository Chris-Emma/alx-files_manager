...
class AuthController {
  // static method to handle user login and token generation
  static async getConnect(req, res) {
    // extract the authorization header from the request
    const authHeader = req.headers.authorization;

    // checks if auth header is missing or doesn't start with 'Basic '
    if (!authHeader || !authHeader.startsWith('Basic ')) {
      return res.status(401).json({ error: 'Unauthorized' });
    }

    // extracts the base64 encoded credentials from the auth header
    const base64Credentials = authHeader.split(' ')[1];
    const credentials = Buffer.from(base64Credentials, 'base64').toString(
      'ascii',
    );
    // splits decoded credentials into email and password
    ...
    const hashedPassword = sha1(password); // hashes the provided password
    // searches for a user with the provided email & hashed password
    const user = await dbClient.db
      .collection('users')
      .findOne({ email, password: hashedPassword });

    if (!user) {
      // returnes 401 if no user is found with the provided email & password
      return res.status(401).json({ error: 'Unauthorized' });
    }

    const token = uuidv4(); // generate unique token using uuidv4
    const tokenKey = `auth_${token}`; // create key for storing token in redis
    // stores user's ID in redis with generated token as key to expire in 24 hours
  ...
  static async getDisconnect(req, res) {
    // extractes token from the x-token header
